...
import static com.google.flatbuffersext.Constants.FILE_IDENTIFIER_LENGTH;
import static com.google.flatbuffersext.Constants.SIZEOF_INT;
import static com.google.flatbuffersext.Constants.SIZEOF_SHORT;
...
public class FlatBufferBuilder {
    ...
    static final Charset utf8charset = Charset
            .forName("UTF-8"); // The UTF-8 character set used by FlatBuffers.
    ...
    /**
     * Alternative constructor allowing reuse of {@link ByteBuffer}s.  The builder can still grow
     * the buffer as necessary.  User classes should make sure to call {@link #dataBuffer()} to
     * obtain the resulting encoded message.
     *
     ...
     * @return A new byte buffer with the old data copied copied to it.  The data is located at the
     * end of the buffer.
     */
     ...
     * method in which the internal buffer gets allocated. This allows for alternatives to the
     * default behavior, which is to allocate memory for a new byte-array backed `ByteBuffer` array
     * inside the JVM.
     * <p>
     * The FlatBufferBuilder class contains the HeapByteBufferFactory class to preserve the default
    ...
    public interface ByteBufferFactory {
        ...
        /**
         * Release a ByteBuffer. Current {@link FlatBufferBuilder} released any reference to it, so
         * it is safe to dispose the buffer or return it to a pool. It is not guaranteed that the
         * buffer has been created with {@link #newByteBuffer(int) }.
         *
